...
class THEMECOMPLETE_EPO_Display {
	...
	public function tm_epo_fields( $product_id = 0, $form_prefix = '', $is_from_shortcode = false, $dummy_prefix = false ) {
		...
		// Around line 850-900, modify the tm-start.php template call:

wc_get_template(
    'tm-start.php',
    [
        'isfromshortcode'      => $isfromshortcode,
        'classcart'            => $classcart . 'tm-builder-tabs-right', // Add the layout class here
        'forcart'              => $forcart,
...
echo '<div class="tc-tabs">';
echo '<div class="tc-tabs-wrap tcwidth tcwidth-100">';
echo '<div class="tc-tab-headers tcwidth tcwidth-100">';

foreach ( $sections_tabs_labels as $tab_index => $tab_label ) {
    echo '<div class="tc-tab-header tma-tab-label">';
    echo '<div tabindex="0"
            data-id="tc-tab-slide' . esc_attr( $tab_index ) . '"
            data-tab="tab' . esc_attr( $tab_index ) . '"
            class="tab-header' . ( 0 === $tab_index ? ' open' : '' ) . '">';
    echo '<span class="tab-header-label">';
    echo apply_filters( 'wc_epo_kses', wp_kses_post( $tab_label ), $tab_label, false );
    echo '</span></div></div>';
}

echo '</div>'; // tc-tab-headers
echo '</div>'; // tc-tabs-wrap
echo '</div>'; // tc-tabs
 ...
 * OPEN NEW LAYOUT
 * =========================== */
echo '<div class="tm-extra-product-options tm-builder-tabs-right">';
